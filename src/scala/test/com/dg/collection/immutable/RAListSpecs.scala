...
class RAListSpecsTest extends org.specs.runner.JUnit4(RAListSpecs)
object RAListSpecs extends Specification with Scalacheck {
  import Prop._
  
  val emptyList: RAList[Int] = RAList.empty
  ...
  "random access list" should {
    ...
    "store lots of elements" in {
      val LENGTH = 1000
      val list = (0 until LENGTH).reverse.foldLeft(RAList.empty[Int])((x, y) => y :: x)
